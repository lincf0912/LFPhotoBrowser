...
-(void)initScrubberTimer
{
    ...
    if (isfinite(duration))
    {
        ...
        if ([self.delegate respondsToSelector:@selector(LFPlayerSyncScrub:)]) {
            UISlider *slider = [self.delegate LFPlayerSyncScrub:self];
...
- (void)playerItemDidReachEnd:(NSNotification *)notification
{
    ...
    if ([self.delegate respondsToSelector:@selector(LFPlayerPlayDidReachEnd:)]) {
        [self.delegate LFPlayerPlayDidReachEnd:self];
    }
}
...
-(void)assetFailedToPrepareForPlayback:(NSError *)error
{
    ...
    if ([self.delegate respondsToSelector:@selector(LFPlayerFailedToPrepare:error:)]) {
        [self.delegate LFPlayerFailedToPrepare:self error:error];
    }
                       ...
                       context:(void*)context
{
    ...
    if (context == LFPlayerStatusObservationContext)
    {
        ...
        switch (status)
        {
            ...
            case AVPlayerItemStatusReadyToPlay:
            {
                ...
                if ([self.delegate respondsToSelector:@selector(LFPlayerReadyToPlay:duration:)]) {
                    double duration = 0.0;
                    CMTime playerDuration = [self playerItemDuration];
                    if (!CMTIME_IS_INVALID(playerDuration))
                    {
                    ...
                    [self.delegate LFPlayerReadyToPlay:self duration:duration];
                }
            }
                break;
    ...
    else if (context == LFPlayerCurrentItemObservationContext)
    {
        ...
        else /* Replacement of player currentItem has occurred */
        {
            ...
            if ([self.delegate respondsToSelector:@selector(LFPlayerLayerDisplay:avplayer:)]) {
                [self.delegate LFPlayerLayerDisplay:self avplayer:self.player];
            }
        }
    }
    else
